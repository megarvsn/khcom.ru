...
if (0 < $arResult['SECTIONS_COUNT'])
{
    ...
    $arSelect = array('ID', 'UF_BUTTON_NAME');
    $rsSections = CIBlockSection::GetList(array(), array("IBLOCK_ID" => $arParams["IBLOCK_ID"], "ACTIVE" => "Y", 'ID' => array_keys($arMap)), false, $arSelect);
    while ($arSection = $rsSections->GetNext()) {
        if (!isset($arMap[$arSection['ID']]))
            continue;
        $key = $arMap[$arSection['ID']];
        if ($arSection['UF_BUTTON_NAME']) {
            $arResult['SECTIONS'][$key]['BUTTON_NAME'] = $arSection['UF_BUTTON_NAME'];
        }
    }
}