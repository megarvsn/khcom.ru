	...
	if($arResult = $rsIBlock->GetNext())
	{
		...
		while($obElement = $rsElement->GetNextElement())
		{
			...
			if(strlen($arItem["DATE_ACTIVE_TO"]) > 0)
				$arItem["DISPLAY_DATE_ACTIVE_TO"] = CIBlockFormatProperties::DateFormat($arParams["ACTIVE_DATE_FORMAT"], MakeTimeStamp($arItem["DATE_ACTIVE_TO"], CSite::GetDateFormat()));
			else
				$arItem["DISPLAY_DATE_ACTIVE_TO"] = "";
