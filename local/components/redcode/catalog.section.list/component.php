...
if($this->startResultCache(false, ($arParams["CACHE_GROUPS"]==="N" ? false : $USER->GetGroups())))
{
	...
	$arSelect = array_merge($arSelect, array("ID", "IBLOCK_ID"));
	
	if(array_key_exists("ELEMENT_FIELD_CODE", $arParams) && !empty($arParams["ELEMENT_FIELD_CODE"]) && is_array($arParams["ELEMENT_FIELD_CODE"]))
	{
		foreach($arParams["ELEMENT_FIELD_CODE"] as $field)
		{
			if (!empty($field) && is_string($field))
				$arSelect[] = $field;
		}
	...
	$eGetProperty = count($arParams["ELEMENT_PROPERTY"]) > 0;
	if($eGetProperty)
		$arSelect[] = "PROPERTY_*";
	...
	for($s = 0; $arSection = $rsSections->GetNext(); $s++)
	{
		...
		// Add elements for section
		$arFilter = array_merge($arFilter, array("SECTION_ID" => $arSection["ID"]));	
		
		$arElement = CIBlockElement::GetList(array(), $arFilter, false, false, $arSelect);
		
		for($e = 0; $rsElement = $arElement->GetNextElement(); $e++)
		{
			...
			$arResult["SECTIONS"][$s]["ELEMENTS"][$e] = $arItem;
		}
	}
